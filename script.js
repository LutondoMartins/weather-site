...
const PIXABAY_API_KEY = '49754362-be511f06cc3afffcd9ed009d0'; // Substitua por sua chave da Pixabay API
...
// Obter imagem da cidade da Pixabay API
function getCityImage(city, country) {
    const tryQuery = (query) => {
        return fetch(`https://pixabay.com/api/?key=${PIXABAY_API_KEY}&q=${encodeURIComponent(query)}&image_type=photo&per_page=3`)
            .then(response => {
                if (!response.ok) {
    ...
    tryQuery(`${city} landmark`)
        .catch(() => tryQuery(`${city} cityscape`))
        .catch(() => tryQuery(`${country} landmark`))
        .catch(() => `https://via.placeholder.com/800x500?text=${encodeURIComponent(city)}+Image+Not+Found`)
        .then(imageUrl => {
...
function getWeatherIcon(code) {
    const iconMap = {
        ...
        '10n': `<div class="flex flex-col items-center">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="text-blue-500 w-4 h-4 -mt-2">
                <path fill-rule="evenodd" d="M12 5.25c1.213 0 2.415.046 3.605.135a3.256 3.256 0 013.01 3.01c.044.583.077 1.17.1 1.759L17.03 8.47a.75.75 0 10-1.06 1.06l3 3a.75.75 0 001.06 0l3-3a.75.75 0 00-1.06-1.06l-1.752 1.751c-.023-.65-.06-1.296-.108-1.939a4.756 4.756 0 00-4.392-4.392 49.422 49.422 0 00-7.436 0A4.756 4.756 0 003.89 8.282c-.017.224-.033.447-.046.672a.75.75 0 001.5 0c-.01-.2-.021-.398-.30-.596a3.32 3.32 0 00-.053-.447 3.256 3.256 0 00-3.01-3.01A49.324 49.324 0 0012 5.25z" clip-rule="evenodd" />
            </svg>
        </div>`,
